...
from app.agents.law_library import get_document, get_retrieval_plan, list_documents
...
async def retrieve(query: str = Query(..., min_length=2)):
    ...
    if not plan["cached"]:
        return {
            **plan,
            "message": "Document is not cached yet. Search order is India Code, Indian Kanoon, then PRS Legislative Research.",
